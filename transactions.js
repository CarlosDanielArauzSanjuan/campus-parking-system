...
// -------------------------------------------------
// Registra la hora de salida y costo, y libera el cupo de la zona, todo en una transacción.
function cerrarParqueo(parqueoId) {
  ...
  try {
    ...
    // 1. Buscar parqueo activo por ID
    ...
    // 2. Calcular tiempo transcurrido y costo (ejemplo: $1000/hora)
    const horaSalida = new Date();
    const horas = Math.ceil((horaSalida - parqueo.hora_entrada) / (1000 * 60 * 60));
    const costo = horas * 1000;

    // 3. Actualizar el parqueo
    db.parqueos.updateOne(
      { _id: parqueo._id },
      { $set: { hora_salida: horaSalida, costo: costo } },
      { session }
    ...
    // 4. Liberar un cupo en la zona correspondiente
    const result = db.zonas.updateOne(
      { _id: parqueo.zona_id },
      { $inc: { cupos_disponibles: 1 } },
      { session }
    ...
    session.commitTransaction();
    print(`✅ Parqueo cerrado: ${horas}h, $${costo}`);
